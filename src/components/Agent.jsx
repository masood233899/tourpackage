...
const Agent = () => {
  ...
  const handleDataInputChange = (e) => {
    ...
    if (name === 'duration' && parseInt(value, 10) > 5) {
        // If the value exceeds 5, set it to 5
    ...
    // Show the post button if the value is valid
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < packageData.duration; i++) {
      const packageDetail = {
        ...
        Iterinary: packagedetails[`Iterinary${i}`] || "",
  ...
  const handlePostData = async () => {
    try {
      for (let i = 0; i < packagedetailstodb.length; i++) {
        ...
        formData.append('Iterinary', packageDetail.Iterinary);
        ...
        const response = await axios.post('https://localhost:7290/api/PackageDetailsMasters/PostPlaceMaster', formData);
          ...
          <Grid item xs={12} key={i}>
            <Paper elevation={3} sx={{ padding: 2 }}>
              ...
              <TextField
                ...
                name={`Iterinary${i}`}
                label="Iterinary"
                value={packagedetails[`Iterinary${i}`]}
                onChange={handleDetailsInputChange}
                multiline
                rows={4}
                sx={{ mt: 2 }}
